...
import { getTilesUnderTriangle } from "./roof/utils.js";
...
export default class Tile3DMultipolygon {
    ...
    public getFootprintWithHoles(
        { height, flip }: { height: number; flip: boolean }
    ): {
        positions: number[];
        uvs: number[];
        normals: number[];
        ...
        // 1) On regroupe les anneaux externes et internes
        const inners = this.rings.filter(r => r.type === Tile3DRingType.Inner);
        const outers = this.rings.filter(r => r.type === Tile3DRingType.Outer);

        if (outers.length === 0) {
            return { positions: [], uvs: [], normals: [], holeIndices: [] };
        }

        // Pour l'instant, on ne gère que le premier outer
        const outer = outers[0];

        // 2) getRingEarcutInput prépare flatXY + holeIndices
        //    `vertices` est un array [x0,y0, x1,y1, …]
        //    `holes`    est un array [i0, i1, …] où chaque i<vertices.length/2
        const { vertices, holes } = this.getRingEarcutInput(outer, inners);

        // 3) Remplit positions, uvs et normals à partir de ces vertices
