...
import { BuildingFacadeMaterial, BuildingProperties, BuildingRoofMaterial, BuildingRoofType, RoofType, VectorArea, VectorAreaDescriptor, VectorNode } from "./type.js";
...
import Tile3DRing, { Tile3DRingType, VectorAreaRing } from "./tile-3d-ring.js";
import { BuildingBuilder } from "./building-builder.js";
import { getRoofParams } from "./roof-params.js";
import { getOMBB } from "./ombb-params.js";
import { ExtrudedTextures } from "./roof/textures.js";
import { getBuildingParams } from "./building-params.js";
...
export function createBuildingPolygons(olFeatures: Feature<Polygon>[]) {
    ...
    for (let index = 0; index < olFeatures.length; index++) {
        ...
        if (olFeatureGeometry == null) {
            continue
        }
        const coordinates = olFeatureGeometry.getCoordinates()
        ...
        for (const ring of rings) {
            if (ring.type === VectorAreaRingType.Outer) {
                areas.push({
                    type: 'area',
                    rings: [ring],
                    osmId: olFeature.getProperties()["osmId"],
                    osmType: olFeature.getProperties()["osmType"],
                    descriptor: {
                        type: 'building',
...
export class Builder {
    ...
    private handleBuilding() {
        ...
        if (this.descriptor.label) {
            ...
            const height = facadeMinHeight + (this.descriptor.buildingHeight ?? 0) + 5;
            const labelFeature = {
                type: 'label',
